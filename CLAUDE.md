...
To take the editor down for maintenance, touch a flag file on the app server — no restart or redeploy needed, it's checked on every request:
...
While the file exists, every route returns `storymap/templates/maintenance.html` with a 503 status. That template is self-contained (inline CSS, no calls to the DB, S3, or Flask) so it still renders correctly even if the database is down.
...
The check itself lives in `storymap/api.py` (`check_maintenance_mode`, a `before_request` hook). `.maintenance_mode` is gitignored, so it never ships as part of a deploy.

Note: this only takes every app server out of rotation if there's exactly one (matches the current single-host nginx→gunicorn setup). If StoryMapJS ever moves to multiple app servers behind a load balancer, the flag would need to be set on each one.
